...
export default function CustomVISXStreamgraph({
  width,
  height,
  animate = true,
  l2DataGetter
      ...
      if (data?.simpleAnalysis?.allDatasetsSameLength) {
        ...
        if (data.datasets)
          setProcessedStreamchartData({
            providers: data.datasets.map((x: Dataset) => x.provider as string),
            data: data.datasets.map(
              (x: Dataset) =>
                x.dataPoints
                  ?.slice(0, length)
                  .map((y: DatedXYDataPoint) => y.y as number) ?? []
            )
          })
      }
    }
  ...
  useEffect(() => {
    if (liveData) {
      setDataPoints(liveData.data?.map((x: LiveDataPoint) => x?.value ?? 0))
      ...
      for (let i = 0; i < temp.providers.length; i++) {
        ...
        const v =
          liveData.data?.find(
            (x: LiveDataPoint) => x.providerName === temp.providers[i]
  ...
  }, [liveData, processedStreamchartData])
