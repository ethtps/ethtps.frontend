...
export const diff = (old: LiveDataDelta, newValue: number): LiveDataDelta => {
	return {
		...
		delta: {
			type:
				old.value < newValue ? DeltaType.increase : DeltaType.decrease,
...
export const useLiveDataWithDelta = () => {
	...
	return {
		...
		setTPS: (value: number) =>
			setTPS((prevState) => diff(prevState, value)),
		setGPS: (value: number) => {
...
export type LiveDataWithDeltaReturnType = ReturnType<
	typeof useLiveDataWithDelta
>
