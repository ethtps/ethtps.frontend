...
export function LineChart(props: ILineChartProps): JSX.Element {
	...
	const [dataType, setDataType] = useState<ETHTPSDataCoreDataType>(
		...
		async function fetchData(dataType: ETHTPSDataCoreDataType) {
			try {
				...
				const processed = {} as Record<
					...
					ETHTPSDataCoreModelsDataPointsXYPointsNumericXYDataPoint[]
				>
				processed[props.provider ?? 'auto'] =
					data?.data?.dataPoints?.map(
						(
							x: ETHTPSDataCoreModelsDataPointsXYPointsNumericXYDataPoint,
							i: number
						) => {
							return {
								x: x.x ? x.x : i,
				...
				setAllData((old) => {
					switch (dataType) {
						case ETHTPSDataCoreDataType.TPS:
							old.tps = processed as Record<
								string,
								ETHTPSDataCoreModelsDataPointsXYPointsNumericXYDataPoint[]
							>
							break
						case ETHTPSDataCoreDataType.GPS:
							old.gps = processed as Record<
								string,
								ETHTPSDataCoreModelsDataPointsXYPointsNumericXYDataPoint[]
							>
							break
						case ETHTPSDataCoreDataType.GAS_ADJUSTED_TPS:
							old.gtps = processed as Record<
								string,
								ETHTPSDataCoreModelsDataPointsXYPointsNumericXYDataPoint[]
							>
							break
