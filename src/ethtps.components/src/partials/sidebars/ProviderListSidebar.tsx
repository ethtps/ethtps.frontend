...
const createProviderButton = (
	...
	<Button
		...
		rightIcon={
			hasRightIcon ? <ChevronRightIcon color={colors.text} /> : undefined
		}
		leftIcon={
			hasLeftIcon ? <ChevronLeftIcon color={colors.text} /> : undefined
		}>
		<Image
		...
		/>
		<Text color={colors.text}>{provider.name}</Text>
	</Button>
)
...
export const ProviderListSidebar: React.FC<SidePanelProps> = ({
	allProviders,
	currentProvider,
	...
	const groupedProviders = groupBy(
		allProviders,
