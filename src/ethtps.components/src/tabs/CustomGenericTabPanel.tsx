...
export function CustomGenericTabPanel({ items, localStorageKey }: GenericTabPanelProps): JSX.Element {
    const generateKey = (tab: string) => `${localStorageKey}${tab}`
    const [tabIndex, setTabIndex] = useState(0)

    const handleTabChange = (index: number) => {
        setTabIndex(index)
        localStorage.setItem(generateKey(items[index].title), JSON.stringify(index))
    }
